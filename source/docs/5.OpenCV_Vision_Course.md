...
class Camera(Node):
    def __init__(self, name):
        ...
        # Register the Ctrl+C interrupt signal handler（注册 Ctrl+C 中断信号处理器）
...
```
    # Process ROS Image messages and convert them to OpenCV format（处理 ROS Image 消息并转化为 OpenCV 格式）
    def image_callback(self, ros_image):
        try:
            # Convert ROS Image messages to OpenCV images（将 ROS Image 消息转换为 OpenCV 图像）
            self.image = self.bridge.imgmsg_to_cv2(ros_image, "bgr8")

            if self.image_queue.full():
            # If the queue is full, drop the oldest image（如果队列已满，丢弃最旧的图像）
...
class Camera(Node):
    def __init__(self, name):
        ...
        # Create a CvBridge object to convert ROS Image to OpenCV format（创建 CvBridge 对象，用于将 ROS Image 转为 OpenCV 格式）
        self.bridge = CvBridge()
        self.target_color = "red"  # Set the target color（设置目标颜色）
...
```
    ...
    def image_callback(self, ros_image):
        ...
        if self.image_queue.full():
            # If the queue is full, drop the oldest image（如果队列已满，丢弃最旧的图像）
            try:
                self.image_queue.get_nowait() # Use get_nowait to avoid blocking（使用 get_nowait 避免阻塞）
            except queue.Empty:
                pass # Ignore the exception if the queue is already empty（如果队列在检查时已经为空，则忽略异常）
        # Put the image into the queue（将图像放入队列）
        try:
            self.image_queue.put_nowait(bgr_image)  # Use put_nowait to avoid blocking（使用 put_nowait 避免阻塞）
        except queue.Full:
            pass # Ignore the exception if the queue is already full（如果队列在检查时已经为满，则忽略异常）
        if not self.thread_started:
            # Start the thread only on the first invocation（只在第一次调用时启动线程）
...
```
    def image_process(self):
        while self.running:
            try:
                bgr_image = self.image_queue.get(block=True, timeout=1) # Add try-except when using get（使用 get 时添加 try-except）
            except queue.Empty:
            ...
            # Apply color thresholds and binarize; default is red（设置颜色阈值并进行二值化，默认为红色）
            ...
            # Publish the processed image（发布处理后的图像）
            try:
...
class ColorDetection(Node):
    def __init__(self, name):
        ...
        self.bridge = CvBridge()  # Used for converting between ROS Image messages and OpenCV images（用于ROS Image消息与OpenCV图像之间的转换）
        ...
        # Subscribe to the image topic（订阅图像话题）
        self.image_sub = self.create_subscription(Image, '/depth_cam/rgb/image_raw', self.image_callback, 1)

        # Publish the color recognition result image（发布颜色识别处理图像）
...
```
    # Process ROS Image messages and convert them to OpenCV images（处理ROS Image消息，将其转换为OpenCV图像）
    def image_callback(self, ros_image):
        try:
            ...
            # Display the original image and detection result（展示原图和检测结果）
...
```
    # Perform erosion and dilation（腐蚀与膨胀操作）
...
```
    ...
    def color_detection(self, color, bgr_image):
        ...
        # Apply color thresholds to generate a binary image（设置颜色阈值并生成二值化图像）
        ...
        # Publish the BGR image（发布彩色 BGR 图像）
    ...
    def image_callback(self, result_image):
        try:
            ...
            if result_image is not None :
                # Compute the detected contours（计算识别到的轮廓）
                ...
                if contours :
                    ...
                    c = max(contours, key = cv2.contourArea)
                    # Decide whether to continue based on the contour size（根据轮廓大小判断是否进行下一步处理）
                    rect = cv2.minAreaRect(c)  # Obtain the minimum bounding rectangle（获取最小外接矩形）
                    yaw = int(round(rect[2]))  # Rectangle angle（矩形角度）
                    # Print the object pose（打印物体姿态）
...
```
    ...
    def image_callback(self, result_image):
        try:
            ...
            if result_image is not None :
                # Calculate detected contours (计算识别到的轮廓)
                contours = cv2.findContours(result_image, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)[-2]  # Find all contours (找出所有轮廓)
                ...
                if contours :
                    ...
                    # Find the largest contour (找出最大轮廓)
                    c = max(contours, key = cv2.contourArea)
                    # Determine whether to proceed based on the contour size (根据轮廓大小判断是否进行下一步处理)
                    rect = cv2.minAreaRect(c)  # Get the minimum bounding rectangle (获取最小外接矩形)
                    yaw = int(round(rect[2]))  # Rectangle angle (矩形角度)
                    # Print object posture (打印物体姿态)
                    self.get_logger().info(f" 物体姿态:,yaw: {yaw}")
...
```
    def __init__(self, name):
        ...
        self.bridge = CvBridge()  # Bridge for converting between ROS Image and OpenCV image (ROS Image和OpenCV图像转换桥梁)
        self.K = None  # Camera intrinsic matrix (相机内参矩阵)
        self.white_area_center = None  # Center point of the white area in world coordinates (白色区域中心点（世界坐标系）)
        self.camera_type = os.environ.get('CAMERA_TYPE', '')  # Read CAMERA_TYPE from environment variables, empty string if not found (读取CAMERA_TYPE环境变量，若无则空字符串)
        self.config_file = 'transform.yaml'  # Configuration file for pose calculation (位姿计算配置文件名)
        self.calibration_file = 'calibration.yaml'  # Calibration file name (校准文件名)
        self.config_path = "/home/ubuntu/ros2_ws/src/app/config/"  # Directory for configuration files (配置文件目录)

        # Subscribe to image result topic and camera intrinsic topic (订阅图像结果话题和相机内参话题)
...
```
    def get_yaml(self):
        """Read the configuration file to obtain camera extrinsics and the world coordinates of the white area (读取配置文件，获取相机外参和白色区域世界坐标)"""
        with open(os.path.join(self.config_path, self.config_file), 'r') as f:
            config = yaml.safe_load(f)
            extristric = np.array(config['extristric'])
            corners = np.array(config['corners']).reshape(-1, 3)  # Corner points, currently unused (角点，目前没有用到)
            self.white_area_center = np.array(config['white_area_pose_world'])

        tvec = extristric[:1]  # Translation vector (first row) (第一行平移向量)
        rmat = extristric[1:]  # Rotation matrix (remaining three rows) (剩余三行旋转矩阵)

        # Call the external parameter translation offset calculation function, requires np.matrix type (调用外参平移偏移计算函数，要求传入np.matrix类型)
...
```
    def camera_info_callback(self, msg):
        """Triggered when a camera intrinsic message is received, convert to a 3x3 matrix (收到相机内参消息时触发，转换成3x3矩阵)"""
        # Use np.matrix here to ensure no error occurs when calling common (这里用np.matrix，确保调用common时不会报错)
...
```
    def image_callback(self, result_image):
        """Triggered when an image message is received, detects the target and calculates its actual position (收到图像消息时触发，识别目标，计算实际位置)"""
        try:
            cv_image = self.bridge.imgmsg_to_cv2(result_image, "mono8")  # Convert to grayscale image (转为灰度图)

            if cv_image is not None:
                contours = cv2.findContours(cv_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[-2]

                if contours:
                    # Find the largest contour (找最大轮廓)
                    c = max(contours, key=cv2.contourArea)
                    ...
                    if self.camera_type == 'usb_cam':
                        # Placeholder for usb_cam-specific correction logic, left empty to avoid errors (这里可填usb_cam专用校正逻辑，先留空避免报错)
                        pass

                    # Load extrinsic parameters from configuration (读取外参配置)
                    self.get_yaml()

                    # Assemble projection matrix, remember to convert to np.matrix (组装投影矩阵，记得转成np.matrix)
                    ...
                    # Convert pixel coordinates to world coordinates, pass parameters as np.matrix type (将像素坐标转换到世界坐标系，传入np.matrix类型参数)
                    world_pose = common.pixels_to_world([[x, y]], self.K, projection_matrix)[0]

                    # Adjust coordinate system (做坐标系调整)
                    world_pose[0] = -world_pose[0]
                    world_pose[1] = -world_pose[1]

                    # Add the white area center coordinate to calculate the actual position (加上白区中心坐标，计算真实坐标)
                    position = self.white_area_center[:3, 3] + world_pose
                    position[2] = 0.03  # Fixed height (固定高度)

                    # Read calibration parameters for scaling and offset (读取标定参数做缩放和位移)
                    ...
                    # Print pixel and actual coordinates (打印像素及实际坐标)
...
class PathPlanning(Node):
    def __init__(self, name):
        ...
        bus_servo_control.set_servo_position(self.servos_pub, 1.0, ((6, self.servos_list[0]), (5, self.servos_list[1]), (4, self.servos_list[2]), (3, self.servos_list[3]), (2, self.servos_list[4]), (1, self.servos_list[5])))  # Set the initial position of the robotic arm (设置机械臂初始位置)
        self.run()
```
        ...
        while rclpy.ok():
            if future.done():
                try:
                    ...
                    return None
            rclpy.spin_once(self)  # Allow the node to handle callback functions (允许节点处理回调函数)
```
...
```
    def move_0(self, x, y, z, pitch, t=1000):
        # Check if the robotic arm can plan motion to the set coordinates (是否可以规划机械臂运动到设置的坐标)
...
```
    def start_sortting(self, pose_t, pose_R):
        ...
        if res.pulse:  # Reachable (可以达到)
            ...
            bus_servo_control.set_servo_position(self.servos_pub, 1.0, ((6, 500), (5, 600), (4, 800), (3, 100), (2, 500), (1, 210)))  # Set the robotic arm to initial position (设置机械臂初始位置)
            time.sleep(2)
                ...
                if self.result_image is not None :
                    ...
                    if contours :
                        ...
                        if self.count >= 60:  # If the counter reaches the threshold, consider the position stable. （如果计数器达到阈值，则认为位置已稳定）
                            ...
                            self.start_sortting(position,yaw)
                            self.count = 0
                            break
```
...
```
    def get_roi(self):
        ...
        tvec = extristric[:1]  # Take the first row (取第一行)
        rmat = extristric[1:]  # Take the remaining three rows (取后面三行)
        ...
        # Crop the ROI region (裁切出ROI区域)
        x_min = min(imgpts, key=lambda p: p[0])[0] # Minimum value on x-axis (x轴最小值)
        x_max = max(imgpts, key=lambda p: p[0])[0] # Maximum value on x-axis (x轴最大值)
        y_min = min(imgpts, key=lambda p: p[1])[1] # Minimum value on y-axis (y轴最小值)
        y_max = max(imgpts, key=lambda p: p[1])[1] # Maximum value on y-axis (y轴最大值)
                    ...
                    if len(tags) > 0:
                        ...
                        for tag in tags:
                            if 'tag%d'%tag.tag_id in self.target_labels:
                                ...
                                # rect contains (center, (width, height), rotation angle) (rect 包含 (中心点, (宽度, 高度), 旋转角度))
                                (center, (width, height), angle) = rect
                                index += 1
                                target_info.append(['tag%d'%tag.tag_id, index, (int(center[0]), int(center[1])), (int(width), int(height)), angle])
                    if target_info:
                        if self.last_object_info_list:
                            # Compare with the positions of the objects from the previous frame to reorder (对比上一次的物体的位置来重新排序)
...
```
    def image_callback(self, ros_rgb_image):
        # Convert ROS image to OpenCV format (将ros格式图像转换为opencv格式)
...
```
    def __init__(self, name):
        ...
        Heart(self, self.name + '/heartbeat', 5, lambda _: self.exit_srv_callback(request=Trigger.Request(), response=Trigger.Response()))  # Heartbeat signal (心跳包)
        self.kinematics_client = self.create_client(SetRobotPose, '/kinematics/set_pose_target')
        self.kinematics_client.wait_for_service()
        ...
        if not self.get_parameter('broadcast').value: 
            ...
            req.data = 1.0 # Tracked color: 0.0 for no tracking, 1.0 for red, 2.0 for green, 3.0 for blue (追踪的颜色 0.0为不追踪， 1.0为红色， 2.0为绿色，3.0为蓝色)
...
```
    def enter_srv_callback(self, request, response):
        ...
        thread = threading.Thread(target=self.goback).start()
        self.image_sub = self.create_subscription(Image, '/depth_cam/rgb/image_raw', self.image_callback, 10)  # Subscribe to the camera(摄像头订阅)
        with self.lock:
            self.enable_face_tracking = False
            self.enable_color_tracking = False
        self.thread_started = False
...
```
    def image_callback(self, ros_image):
        # Convert ROS format image to OpenCV format (将ros格式图像转换为opencv格式)
...
class TagTrackNode(Node):
    def __init__(self, name):
        ...
        self.create_service(Trigger, '~/start', self.start_srv_callback) # Enter play mode (进入玩法)
        self.create_service(Trigger, '~/stop', self.stop_srv_callback, callback_group=timer_cb_group) # Exit play mode (退出玩法)
        self.client = self.create_client(Trigger, '/controller_manager/init_finish')
        self.client.wait_for_service()
        self.client = self.create_client(Trigger, '/kinematics/init_finish')
        self.client.wait_for_service()
