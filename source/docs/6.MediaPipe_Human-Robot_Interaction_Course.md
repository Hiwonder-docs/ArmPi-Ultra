...
<img class="common_img" src="../_static/media/chapter_6/media/image4.png" style="width:500px" />
...
<img class="common_img" src="../_static/media/chapter_6/media/image5.png" style="width:500px" />
...
2. Click the terminal icon <img src="../_static/media/chapter_6/media/image6.png" style="width:50px" /> in the system desktop to open a command-line window. Enter the following command and press **Enter** to stop the auto-start service.
...
```
    def enter_srv_callback(self, request, response):
        ...
        self.send_request(self.start_yolov8_client, Trigger.Request())   # Start YOLOv8 detection on enter（在enter时启动YOLOv8检测）
        self.enter = True
    ...
    def exit_srv_callback(self, request, response):
        if self.enter:
            if self.image_sub is not None:
                ...
                self.object_sub = None
                self.camera_info_sub = None
            self.send_request(self.stop_yolov8_client, Trigger.Request())  # Stop YOLOv8 detection on exit（退出时停止YOLOv8检测）
            self.heart.destroy()
            self.heart = None
...
```
    def main(self):
        while self.running:
            ...
            resize_image = cv2.resize(image, (int(image.shape[1] / 2), int(image.shape[0] / 2)), cv2.INTER_NEAREST) # Resize the image（缩放图片）
            results = self.face_mesh.process(resize_image) # Perform face detection（调用人脸检测）
...
<img class="common_img" src="../_static/media/chapter_6/media/image47.png" style="width:500px" />
...
class FaceTrackingNode(Node):
    def __init__(self, name):
        ...
        self.image_sub = self.create_subscription(Image, 'depth_cam/rgb/image_raw', self.image_callback, 1)  # Camera subscription（摄像头订阅）

        self.result_publisher = self.create_publisher(Image, '~/image_result', 1)  # Publish image processing results（图像处理结果发布）
        timer_cb_group = ReentrantCallbackGroup()
        self.create_service(Trigger, '~/start', self.start_srv_callback, callback_group=timer_cb_group) # Enter mode（进入玩法）
        self.create_service(Trigger, '~/stop', self.stop_srv_callback, callback_group=timer_cb_group) # Exit mode（退出玩法）
        self.client = self.create_client(Trigger, 'controller_manager/init_finish')
        self.client.wait_for_service()
        self.client = self.create_client(Trigger, 'kinematics/init_finish')
        self.client.wait_for_service()
    ...
    def main(self):
        ...
        while self.running:
            ...
            if len(boxes) > 0:
                ...
                self.detected_face = min(self.detected_face, 20) # Keep the counter no greater than 20（让计数总是不大于20）
                # Start tracking after detecting faces for 5 consecutive frames to avoid false positives（连续 5 帧识别到了人脸就开始追踪, 避免误识别）
                if self.detected_face > 5 and self.face_target is None:
                    center = [box_center(box) for box in boxes] # Compute the center coordinates of all faces（计算所有人脸的中心坐标）
                    dist = [distance(c, (o_w / 2, o_h / 2)) for c in center] # Compute distances from face centers to the screen center（计算所有人脸中心坐标到画面中心的距离）
                    face = min(zip(boxes, center, dist), key=lambda k: k[2]) # Find the face closest to the screen center（找出到画面中心距离最小的人脸）
                result_image = show_faces(result_image, bgr_image, boxes, keypoints) # Display detected faces and facial key points（在画面中显示识别到的人脸和脸部关键点）
            else: # Handling when no face is detected（这里是没有识别到人脸的处理）
...
<img class="common_img" src="../_static/media/chapter_6/media/image57.png" style="width:500px" />
...
<img class="common_img" src="../_static/media/chapter_6/media/image62.png" style="width:500px" />
...
class HandGestureNode(Node):
    def __init__(self, name):
        ...
        self.bridge = CvBridge()  # Convert between ROS Image messages and OpenCV images（用于ROS Image消息与OpenCV图像之间的转换）
        self.buzzer = buzzer.BuzzerController()  # Buzzer controller（蜂鸣器控制器）
        self.image_queue = queue.Queue(maxsize=2)
        self.running = True
        self.draw = True
...
```
    def main(self):
        while self.running:
            ...
            bgr_image = self.image_queue.get()
            bgr_image = cv2.flip(bgr_image, 1) # Mirror image（镜像画面）
...
<img class="common_img" src="../_static/media/chapter_6/media/image75.png" style="width:500px" />

When you open all five fingers, the trajectory map will be recorded, and the detected shape will be displayed.

<img class="common_img" src="../_static/media/chapter_6/media/image76.png" style="width:500px" />
...
def get_hand_landmarks(img, landmarks):
    """
    Convert landmarks from the normalized output of mediapipe to pixel coordinates (将landmarks从medipipe的归一化输出转为像素坐标)
...
def draw_points(img, points, tickness=4, color=(255, 0, 0)):
    """
    Draw lines connecting the recorded points on the screen (将记录的点连线画在画面上)
    """
...
def get_track_img(points):
    """
    Generate a trajectory image with a black background and white lines using the recorded points (用记录的点生成一张黑底白线的轨迹图)
    """
    points = np.array(points).astype(dtype=np.int32)
        ...
        while self.running:
            ...
            try:
                ...
                if results is not None and results.multi_hand_landmarks:
                    gesture = "none"
                    index_finger_tip = [0, 0]
                    self.no_finger_timestamp = time.time()  # Note the current time for timeout handling (记下当期时间，以便超时处理)
                    for hand_landmarks in results.multi_hand_landmarks:
                        self.drawing.draw_landmarks(
                            result_image,
                            hand_landmarks,
                    ...
                    if self.state == State.NULL:
                        if gesture == "one":  # Detect the index finger extended while the other fingers are clenched into a fist (检测到单独伸出食指，其他手指握拳)
                            self.start_count += 1
                        ...
                        if gesture == "five": # Finish drawing when all five fingers are spread out (伸开五指结束画图)
                            self.state = State.NULL

                            # Generate a black and white trajectory image (生成黑白轨迹图)
                            ...
                            # Identify the drawn shape based on the trajectory image (按轨迹图识别所画图形)
                            ...
                            print(len(approx))
                            # Determine the shape based on the number of vertices in the contour envelope (根据轮廓包络的顶点数确定图形)
                            if len(approx) == 3:
            ...
            if key == ord(' '): # Press the space key to clear the recorded trajectory. (按空格键清空已记录的轨迹)
                self.points = []
```
...
<img class="common_img" src="../_static/media/chapter_6/media/image88.png" style="width:500px" />
