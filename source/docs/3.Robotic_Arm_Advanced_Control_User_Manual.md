...
def main(args=None):
    ...
    fk_demo_node = FkDemo()  # Create the node（创建节点）
    rclpy.spin(fk_demo_node)  # Keep the node running（保持节点运行）
    fk_demo_node.destroy_node()  # Destroy the node（销毁节点）
    rclpy.shutdown()  # Shut down rclpy（关闭 rclpy）
```
...
class IkDemo(Node):
    def __init__(self):
        ...
        # Publish the bus servo topic（发布总线舵机话题）
        self.servos_pub = self.create_publisher(ServosPosition, '/servo_controller', 1)

        # Wait for the underlying robotic arm control services to start（等待机械臂底层控制服务启动）
        self.client = self.create_client(Trigger, '/controller_manager/init_finish')
        self.client.wait_for_service()
        self.client = self.create_client(Trigger, '/kinematics/init_finish')
        self.client.wait_for_service()
...
```
        # Obtain the inverse kinematics solution（获取逆解）
        ...
        if res != []:
            ...
            # self.set_servo_position(self.servos_pub, self.duration, self.servo_list)# Publish servo information to drive the robotic arm（发布舵机信息，驱动机械臂）
        else:
...
def main(args=None):
    rclpy.init(args=args)  # Initialize rclpy（初始化 rclpy）
    ik_demo_node = IkDemo()  # Create the node（创建节点）
    rclpy.spin(ik_demo_node)  # Keep the node running（保持节点运行）
    ik_demo_node.destroy_node()  # Destroy the node（销毁节点）
    rclpy.shutdown()  # Shut down rclpy（关闭 rclpy）
```
